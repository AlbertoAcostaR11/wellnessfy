...
window.handleCreateChallenge = async function () {
    ...
    if (!name || !goal || !start || !end || sports.length === 0) { window.showToast('Por favor completa todos los campos requeridos', 'error'); return; }
    ...
    if (isNaN(goalNumber) || goalNumber <= 0) {
        window.showToast('La meta debe ser un número positivo', 'error');
        return;
    }
    ...
    const challengeData = {
        ...
        category: mainCategory, // Retrocompatibilidad
        allowedSports: sports,  // NUEVO: Array con todos los permitidos
        metric: `${goal} ${unit}`,
        period: periodText,
