...
import { loadSuggestedUsers } from '../utils/socialSuggestions.js';
...
export function renderCircles(tab = 'discover') {
    ...
    return `
        ...
        <div class="flex items-center justify-between mb-6">
             <h1 class="text-2xl font-bold tracking-tight text-white">Círculos</h1>
        ...
        <!-- Tabs de Navegación -->
        <div class="flex w-full border-b border-white/5 mb-6">
        ...
        <!-- Contenedor Dinámico -->
        <div id="socialContent" class="pb-24 overflow-x-hidden max-w-full">
            ${renderCurrentTab()}
        </div>
    `;
}
...
function initDiscoverView() {
    // 1. Load Suggestions
    setTimeout(() => {
        loadSuggestedUsers().catch(err => console.error('Error init suggestions:', err));
    }, 100);

    // 2. Init Map (if available)
    if (window.initializeExploreMap) {
        setTimeout(window.initializeExploreMap, 300);
    }

    // 3. Mount Sport Selector
    setTimeout(() => {
...
function renderDiscoverTab() {
    ...
    return `
        <div class="space-y-8 relative overflow-x-hidden">
            
            <!-- 1. SEARCH BAR (Sticky Header) -->
            <div class="sticky top-0 z-40 bg-[#050b12]/95 backdrop-blur-xl py-4 mb-4">
            ...
            ${searchResults.length > 0 || isSearching ? `
                <div id="searchResultsSection" class="">
            ...
            ` : `
                ...
                <div class="space-y-3">
                    ...
                    <!-- Carrusel Horizontal -->
                    <div id="suggested-users-container" class="flex gap-4 overflow-x-auto no-scrollbar pb-4 snap-x min-h-[140px] max-w-full">
                        <!-- Loading Skeletons -->
                         <div class="min-w-[140px] h-[140px] bg-white/5 rounded-2xl animate-pulse flex-shrink-0"></div>
                         <div class="min-w-[140px] h-[140px] bg-white/5 rounded-2xl animate-pulse flex-shrink-0"></div>
                         <div class="min-w-[140px] h-[140px] bg-white/5 rounded-2xl animate-pulse flex-shrink-0"></div>
                         <div class="min-w-[140px] h-[140px] bg-white/5 rounded-2xl animate-pulse flex-shrink-0"></div>
                    </div>
                ...
                <!-- Scripts de inicialización movidos a initCircles() -->
                ...
                <div class="space-y-4">
                    ...
                    <!-- Mapa / Radar Visual -->
                    <div id="exploreMap" class="relative w-full max-w-full aspect-video bg-[#0f172a] rounded-2xl border border-white/10 overflow-hidden shadow-inner group cursor-pointer hover:border-[#00f5d4]/30 transition-colors">
...
function renderMyCirclesTab() {
    // Get all unique friends from all circles with their circle memberships
        ...
        if (Array.isArray(circle.members)) {
            circle.members.forEach(memberId => {
                ...
                if (memberId !== currentUserId) {
                    ...
                    friendsMap.get(memberId).circles.push({
                        id: circle.id,
                        name: circle.name,
                        color: circle.neonColor || '#00f5d4'
                    });
                }
            });
        }
    ...
    return `
        <div class="space-y-6">
             <div class="flex items-center justify-between">
                 <h2 class="text-lg font-bold text-white">Tus Círculos</h2>
                ...
                ${AppState.circles.length > 0 ? AppState.circles.map(circle => `
                    <div class="bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col items-center gap-3 hover:bg-white/10 transition-colors cursor-pointer" onclick="showCircleDetail('${circle.id}')">
                        ...
                        <div class="text-center">
                             ...
                             <p class="text-[10px] text-gray-400 uppercase tracking-wider">${Array.isArray(circle.members) ? circle.members.length : (circle.membersList?.length || 1)} Miembros</p>
                        </div>
                    </div>
                `).join('') : `
            ...
            <!-- Todos mis amigos section -->
            ${allFriends.length > 0 ? `
                <div class="space-y-4 mt-8">
                    <h2 class="text-lg font-bold text-white">Todos mis amigos</h2>
                    <div id="all-friends-list" class="space-y-2">
                        ${allFriends.map(friend => `
                            <div class="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition-all" data-friend-id="${friend.id}">
                                <div class="size-10 rounded-full bg-cover bg-center border-2 border-[#00f5d4]/30 bg-gray-700" id="friend-avatar-${friend.id}"></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-bold text-white truncate" id="friend-name-${friend.id}">Cargando...</p>
                                    <div class="flex gap-1 flex-wrap mt-1">
                                        ${friend.circles.map(circle => `
                                            <span class="text-[9px] px-2 py-0.5 rounded-full bg-[${circle.color}]/20 text-[${circle.color}] border border-[${circle.color}]/30">${circle.name}</span>
                                        `).join('')}
                                    </div>
                                </div>
                                <div class="flex gap-2">
                                    <button onclick="navigateTo('profile', document.getElementById('friend-name-${friend.id}').dataset.username)" class="size-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all" title="Ver perfil">
                                        <span class="material-symbols-outlined text-white/60 text-sm">person</span>
                                    </button>
                                    <button onclick="manageFriendCircles('${friend.id}')" class="size-8 rounded-full bg-blue-500/10 hover:bg-blue-500/20 flex items-center justify-center transition-all" title="Gestionar círculos">
                                        <span class="material-symbols-outlined text-blue-400 text-sm">group_add</span>
                                    </button>
                                    <button onclick="removeFriend('${friend.id}')" class="size-8 rounded-full bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center transition-all" title="Eliminar amigo">
                                        <span class="material-symbols-outlined text-red-400 text-sm">person_remove</span>
                                    </button>
                                </div>
                            </div>
                        `).join('')}
            ...
            <div class="p-6 space-y-6 overflow-y-auto">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex gap-3">
                        <button class="flex-1 p-3 rounded-xl bg-[#00f5d4]/20 border-2 border-[#00f5d4] text-white transition-all" id="privacyPublic" onclick="selectCirclePrivacy('public')">
                            <span class="material-symbols-outlined text-2xl mb-1">public</span>
                            <p class="text-[9px] font-bold uppercase">Público</p>
                        </button>
                        <button class="flex-1 p-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 transition-all" id="privacyPrivate" onclick="selectCirclePrivacy('private')">
...
window.selectCirclePrivacy = function (privacy) {
...
window.submitCreateCircle = async function () {
    ...
    try {
        ...
        const newCircle = {
            ...
            members: [AppState.currentUser.uid || AppState.currentUser.id],
...
export async function showCircleDetail(id) {
    ...
    // Handle both legacy (members as number) and new (members as array) structures
    const membersArray = Array.isArray(circle.members)
        ? circle.members
        : (circle.membersList || []);
    const isMember = membersArray.includes(currentUserId);
    const memberCount = membersArray.length || 0;
    ...
    try {
        ...
        for (const memberId of membersArray) {
            try {
                ...
                console.warn(`Could not fetch user ${memberId}:`, error);
            }
        }
    } catch (error) {
            ...
            <div class="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                        <p class="text-2xl font-black text-white">${memberCount}</p>
                ...
                ${memberDetails.length > 0 ? `
                    <div class="space-y-3">
                        <h3 class="text-xs font-black text-white/60 uppercase tracking-wider">Miembros del Círculo</h3>
                        <div class="space-y-2">
                            ${memberDetails.map(member => `
                                <div class="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition-all">
                                    <div class="size-10 rounded-full bg-cover bg-center border-2 border-[#00f5d4]/30" style="background-image: url('${member.avatar || member.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name || 'User')}`}');"></div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-bold text-white truncate">${member.name || 'Usuario'}</p>
                                        <p class="text-xs text-white/40 truncate">@${member.username || 'username'}</p>
                                    </div>
                                    <div class="flex gap-2">
                                        <button onclick="navigateTo('profile', '${member.username}')" class="size-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all" title="Ver perfil">
                                            <span class="material-symbols-outlined text-white/60 text-sm">person</span>
                                        </button>
                                        ${isCreator && member.id !== currentUserId ? `
                                            <button onclick="removeMemberFromCircle('${circle.id}', '${member.id}', '${member.name}')" class="size-8 rounded-full bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center transition-all" title="Remover del círculo">
                                                <span class="material-symbols-outlined text-red-400 text-sm">person_remove</span>
                                            </button>
                                        ` : ''}
                                    </div>
...
window.removeMemberFromCircle = async function (circleId, memberId, memberName) {
    if (!confirm(`¿Seguro que quieres remover a ${memberName} de este círculo?`)) return;
    ...
    try {
        ...
        if (circle && Array.isArray(circle.members)) {
            circle.members = circle.members.filter(id => id !== memberId);
        }

        window.showToast(`${memberName} removido del círculo`, 'success');
        ...
        window.showToast('Error al remover miembro', 'error');
    }
};
