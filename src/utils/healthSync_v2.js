...
import { normalizeSportName } from './sportsDictionaryMaster.js';
import { saveDailySummary } from './dailySummaryManager.js'; // Generador de Fichas Resumen
...
export async function syncHealthData(startDate, endDate) {
    ...
    try {
        ...
        // 🎯 6. Generar Fichas Resumen (Daily Summaries)
        try {
            console.log('📑 Generando Fichas Resumen para Firestore...');
            const userId = AppState.currentUser?.uid || AppState.currentUser?.id;

            if (userId) {
                const { getLocalISOString } = await import('./dateHelper.js');
                const { saveDailySummary } = await import('./dailySummaryManager.js');

                // Usar un Set de fechas únicas encontradas en todas las fuentes de datos
                const uniqueDates = new Set();

                // Añadir fechas de dailyTotals
                dailyTotals.forEach(d => d.date && uniqueDates.add(d.date));

                // Añadir fechas de actividades (FIXED v3: Protección completa en uniqueDates loop)
                normalizedActivities.forEach(a => {
                    let d = '';
                    try {
                        if (a.startTime instanceof Date) d = getLocalISOString(a.startTime);
                        else if (typeof a.startTime === 'string') d = a.startTime.split('T')[0];
                        else if (a.date instanceof Date) d = getLocalISOString(a.date);
                        else if (typeof a.date === 'string') d = a.date.split('T')[0];

                        // Fallback Firestore Timestamp
                        if (!d && a.startTime && typeof a.startTime.toDate === 'function') {
                            d = getLocalISOString(a.startTime.toDate());
                        }
                    } catch (e) { console.warn('Unique dates parsing error:', e); }

                    if (d) uniqueDates.add(d);
                });

                // Si no hay nada, al menos intentar hoy
                if (uniqueDates.size === 0) uniqueDates.add(getLocalISOString(new Date()));

                for (const dateStr of uniqueDates) {
                    // Datos base de dailyTotals si existen
                    const totalData = dailyTotals.find(d => d.date === dateStr) || {};

                    // Filtrar actividades este día (FIXED v2: Protección robusta de fechas)
                    const dayActivities = normalizedActivities.filter(act => {
                        let actDate = '';
                        // Probar múltiples formatos posibles de manera segura
                        try {
                            if (act.startTime instanceof Date) actDate = getLocalISOString(act.startTime);
                            else if (typeof act.startTime === 'string') actDate = act.startTime.split('T')[0];
                            else if (act.date instanceof Date) actDate = getLocalISOString(act.date);
                            else if (typeof act.date === 'string') actDate = act.date.split('T')[0];

                            // Fallback para objetos timestamp de Firestore u otros
                            if (!actDate && act.startTime && typeof act.startTime.toDate === 'function') {
                                actDate = getLocalISOString(act.startTime.toDate());
                            }
                    ...
                    // Buscar sueño
                    const daySleep = sleepHistory.find(s => s.date === dateStr);

                    // Métricas combinadas
                    const metrics = {
                        steps: totalData.steps || (dateStr === getLocalISOString(new Date()) ? todayMetrics.steps : 0),
                        calories: totalData.calories || (dateStr === getLocalISOString(new Date()) ? todayMetrics.calories : 0),
                        distance: totalData.distance || 0,
                        activeMinutes: totalData.activeMinutes || 0
                    };

                    await saveDailySummary(userId, dateStr, metrics, dayActivities, daySleep);
                }
            }
        } catch (error) {
            console.warn('⚠️ Error generador resúmenes:', error);
        }
        return {
            raw: rawActivities,
