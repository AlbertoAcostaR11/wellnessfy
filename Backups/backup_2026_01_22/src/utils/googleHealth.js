...
// Auto-sincronización si hay token válido
...
export function initGoogleIdentity() {
    ...
    try {
        ...
        if (loadSavedToken()) {
            ...
            console.log('🔄 Auto-cargando datos con token guardado...');
            setTimeout(() => {
                fetchFitnessData();
                fetchWeeklyData();
            }, 500);
...
async function fetchWeeklyActivityStats(startDate, endDate, accessToken) {
    ...
    try {
        const [dailyResp, hourlyResp, sleepResp, activitySessionsResp] = await Promise.all([
            ...
            // Nueva petición: TODAS las sesiones de actividad (no solo sueño)
            fetch(`https://www.googleapis.com/fitness/v1/users/me/sessions?startTime=${new Date(startWeekly).toISOString()}&endTime=${new Date(endWeekly).toISOString()}`, {
                method: 'GET',
                headers: { 'Authorization': `Bearer ${accessToken}` }
            })
        ]);
        ...
        const activitySessionsData = await activitySessionsResp.json();
        ...
        // Loguear sesiones de actividad para análisis
        console.log('🏃 Sesiones de Actividad recibidas:', activitySessionsData);
        if (activitySessionsData.session && activitySessionsData.session.length > 0) {
            console.log(`✅ Total de sesiones de actividad: ${activitySessionsData.session.length}`);
            // Mostrar primeras 3 como muestra
            activitySessionsData.session.slice(0, 3).forEach(s => {
                console.log(`   - Tipo: ${s.activityType}, Inicio: ${new Date(s.startTimeMillis).toLocaleString()}, Duración: ${((s.endTimeMillis - s.startTimeMillis) / 60000).toFixed(0)}min`);
            });
        } else {
            console.log('⚠️ No se encontraron sesiones de actividad en los últimos 7 días');
        }

        // Procesar
        const stats = processWeeklyData(hourlyData, dailyData, sleepSessionsData, detailedSleepSegments, activitySessionsData);
...
async function fetchMentalWellbeingData(accessToken) {
    ...
    const body = {
        "aggregateBy": [
            ...
            { "dataTypeName": "com.google.heart_rate.bpm" } // Obtener pulso para RHR fallback
        ],
        "bucketByTime": { "durationMillis": 86400000 },
    ...
    try {
        ...
        if (data.error) {
            console.warn('⚠️ No hay datos de HRV/BPM disponibles:', data.error.message);
            updateStressUI(null);
            return;
        }
        ...
        if (data.bucket) {
            data.bucket.forEach(bucket => {
                ...
                if (bucket.dataset[0]?.point?.length > 0) {
                    ...
                    if (hrv > 0) {
                        // HRV > 100 es excelente (Estrés 0)
                        // HRV < 20 es malo (Estrés 100)
                        // Fórmula inversa simple:
                        dailyScore = Math.max(0, Math.min(100, 100 - (hrv * 0.8)));
                        console.log(`   HRV detectado: ${hrv}ms -> Estrés: ${dailyScore}`);
                    }
                }

                // 2. RHR (Fallback si no hay HRV)
                if (dailyScore === null && bucket.dataset[1]?.point?.length > 0) {
                    // Dataset[1] es BPM. Value[0]=Avg, [1]=Max, [2]=Min
                    const avgBpm = bucket.dataset[1].point[0].value[0].fpVal || 0;
                    const minBpm = bucket.dataset[1].point[0].value[2]?.fpVal || avgBpm; // Usar min como RHR proxy

                    if (minBpm > 0) {
                        // RHR < 50 es excelente, > 80 es estrés
                        // 60bpm = Estrés 40 (Normal)
                        dailyScore = Math.max(0, Math.min(100, (minBpm - 40) * 2));
                        console.log(`   RHR detectado (min BPM): ${minBpm} -> Estrés (Estimado): ${dailyScore}`);
                    }
                }

                if (dailyScore !== null) {
                    totalStress += dailyScore;
                    count++;
                }
            });
        }
        ...
        console.log('✅ Balance Score calculado:', avgStress !== null ? avgStress + '/100' : 'No disponible');
...
function updateStressUI(score) {
    // Si la UI está renderizada por weeklyCharts, esto asegura actualizar el texto
    // Si no, weeklyCharts leerá AppState.
    // Guardamos en AppState para persistencia
    if (!AppState.currentUser.stats.mindfulness) AppState.currentUser.stats.mindfulness = {};
    AppState.currentUser.stats.mindfulness.stress = score;

    // Disparar evento para que UI se entere
    window.dispatchEvent(new CustomEvent('weeklyStatsUpdated', {
        detail: { mindfulness: { stress: score } }
    }));

    // Actualizar UI directa (Fallback)
...
function processWeeklyData(hourlyData, dailyData, sleepSessionData, detailedSleepSegments, activitySessionsData) {
    const stats = {
        activityByHour: [],
        exerciseDays: [],
        sleepData: [],
        ...
        rawActivitySessions: activitySessionsData || { session: [] } // Guardar data cruda para el agregador
    };
    ...
    if (dailyData.bucket && dailyData.bucket.length > 0) {
        dailyData.bucket.forEach(bucket => {
            ...
            // 3. Activity Segments - Procesar para Mindfulness Y para Mis Deportes
            const today = new Date();
            const isToday = date.getDate() === today.getDate() &&
                date.getMonth() === today.getMonth() &&
                date.getFullYear() === today.getFullYear();

            // Calcular minutos de ejercicio real para este día
            ...
            if (bucket.dataset[2]?.point?.length > 0) {
                bucket.dataset[2].point.forEach(point => {
                    ...
                    // Guardar TODOS los segments para el agregador de deportes
                    ...
                    // Excluir sleep segments (1-6), mindfulness (45, 100, 106, 115), 
                    // actividades pasivas (0=in vehicle, 108=other si es muy corto)
                    const excludedTypes = [0, 1, 2, 3, 4, 5, 6, 45, 100, 106, 115];

                    if (!excludedTypes.includes(activityType)) {
                        // Contar como ejercicio real
                        exerciseMinutes += duration / 60; // Convertir segundos a minutos
                    }

                    // Procesar mindfulness solo para HOY
                    if (isToday) {
                        if (activityType === 100) stats.mindfulness.yoga += duration;
                        else if (activityType === 45) stats.mindfulness.meditation += duration;
                        else if (activityType === 15 || activityType === 106 || activityType === 115) stats.mindfulness.breathing += duration;
                    }
                });
            }
            ...
            // Un día cuenta como "día de ejercicio" si:
            // - Tiene más de 5000 pasos, O
            // - Tiene al menos 20 minutos de actividad deportiva real
            const hasExercise = steps > 5000 || exerciseMinutes >= 20;
...
export function autoSyncIfReady() {
    console.log('🔄 Verificando auto-sync...');
    if (localStorage.getItem('google_health_token')) {
        initGoogleIdentity();
    }
}
window.autoSyncIfReady = autoSyncIfReady;
