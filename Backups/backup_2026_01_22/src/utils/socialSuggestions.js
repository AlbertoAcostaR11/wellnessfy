...
export async function loadSuggestedUsers() {
    ...
    try {
        ...
        // Get all friend IDs from user's circles
        const friendIds = new Set();
        if (AppState.circles && Array.isArray(AppState.circles)) {
        ...
        console.log(`🔍 Found ${friendIds.size} existing friends to exclude from suggestions`);

        // Filter out: current user AND existing friends
        const filteredUsers = users.filter(user => {
            // Exclude current user
            if (user.id === currentUserId || user.name === currentUserName) {
                return false;
            }
            // Exclude existing friends
            if (friendIds.has(user.id)) {
                console.log(`⏭️  Skipping ${user.name} - already a friend`);
                return false;
            }
            return true;
        });
        ...
        if (filteredUsers.length > 0) {
            ...
            console.log(`✅ Showing ${filteredUsers.length} suggestions (excluded ${users.length - filteredUsers.length} friends)`);
        } else {
