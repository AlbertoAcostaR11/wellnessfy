                            ...
                            <div class="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer" onclick="window.viewUserProfile('${user.uid}')">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        ${user.isPublic === false ? '<span class="material-symbols-outlined text-white/30 text-xs shadow-sm">lock</span>' : ''}
                                    </div>
                                    <div class="text-[10px] text-[#00f5d4] font-bold tracking-tight mb-1">${user.username || '@usuario'}</div>
                                    ${user.bio ? `<div class="text-xs text-white/60 line-clamp-1">${user.bio}</div>` : ''}
                                </div>
                                <button 
...
window.searchUsers = async function () {
    ...
    try {
        ...
        allUsersSnapshot.forEach((doc) => {
            ...
            const matchEmail = userEmail.includes(query);
            const matchName = userName.includes(query);
            const matchUser = userUsername.includes(query) || userUsername.replace('@', '').includes(query);

            if (matchEmail || matchName || matchUser) {
                results.push({
                    uid: doc.id,
                    ...userData
                });
...
window.sendFriendRequest = async function (targetUserId) {
    ...
    try {
        ...
        await addDoc(collection(db, 'friendRequests'), {
            from: currentUser.uid || currentUser.id,
        ...
        // Actualizar UI del botón para dar feedback inmediato
        const btn = document.querySelector(`button[onclick*="sendFriendRequest('${targetUserId}')"]`);
        if (btn) {
            btn.innerHTML = `
                <div class="flex items-center justify-center gap-2">
                    <span class="material-symbols-outlined text-sm">done</span>
                    <span class="text-[10px] font-black uppercase tracking-wider">Solicitud enviada</span>
                </div>
            `;
            btn.classList.remove('bg-[#00f5d4]/20', 'text-[#00f5d4]', 'size-10');
            btn.classList.add('bg-white/10', 'text-white/40', 'px-4', 'w-auto', 'rounded-xl');
            btn.disabled = true;
        }
