...
// Renderizar Sueño Semanal (Semana Actual - Lunes a Domingo)
        ...
        return `
            <div class="flex-1 flex flex-col items-center justify-end" style="height: 100%;">
                <div class="w-full rounded-t-lg transition-all hover:opacity-80" 
                     ...
                     title="${hourData.hour}:00 - ${Math.floor(Math.round(minutes) / 60)}h ${Math.round(minutes) % 60}m activos"></div>
            </div>
        `;
    }).join('');
...
function renderWeeklyTotals(totals) {
    ...
    if (distanceEl) distanceEl.textContent = totals.distance.toFixed(1);
