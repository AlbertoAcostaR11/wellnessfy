/* src/pages/activity.js - FIXED & CLEANED v3 - Updated 2026-01-22 18:27 */
...
function renderResumenTab() {
    ...
    const goalsHTML = AppState.goalsData ? (() => {
        ...
        const dailyGoals = [
            ...
            { key: 'dailySleepHours', current: (todayStats.sleep || 0) / 60 },
        ...
        const totalProgress = enabledGoals.reduce((sum, g) => {
            const goal = goals[g.key];
            const progress = goal.target > 0 ? Math.min((g.current / goal.target) * 100, 100) : 0;
            return sum + progress;
...
function renderDeportesTab() {
    ...
    // Labels de días (Lun-Hoy de la semana actual)
    const weekDates = getCurrentWeekDays();
    // console.log('DEBUG: weekDates in renderDeportesTab:', weekDates);
    const dayLabels = weekDates.map(dateStr => {
    ...
    const cardsHtml = sportsList.map(sportName => {
        ...
        // Promedio diario basado en los días transcurridos de la semana (Lunes-Hoy)
        const daysElapsed = weekDates.length || 1;
        const dailyAvg = totalValue / daysElapsed;
        ...
        const barsHtml = data.days.map((dayData, idx) => {
            ...
            // Verificar si es HOY usando la fecha real
            const isToday = weekDates[idx] === getLocalISOString();
...
function aggregateSportsData(activities) {
    ...
    // Generar array de fechas de la SEMANA ACTUAL (Lun-Hoy)
    ...
    activities.forEach(act => {
        ...
        // Solo procesar si está en la semana actual (Lun-Hoy)
        if (dayIndex >= 0) {
            ...
            if (!sports[sportName]) {
                sports[sportName] = {
                    days: Array(dates.length).fill(null).map(() => ({ duration: 0, distance: 0 })),
                    totalDuration: 0,
                    totalDistance: 0,
                    meta: meta,
                    count: 0
