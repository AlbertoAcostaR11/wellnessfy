...
import { db, doc, updateDoc, increment, addDoc, collection } from '../config/firebaseInit.js';
...
// Firebase Storage imports (storage not in firebaseInit.js yet)
import { getStorage, ref, uploadBytes, getDownloadURL } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-storage.js';

// Get storage instance (db comes from firebaseInit.js)
...
window.publishPost = async function () {
    ...
    try {
        ...
        const newPost = {
            author: {
                id: AppState.currentUser.uid, // Required for notifications
...
async function reactToPost(postId, type) {
    ...
    post.reactions[type]++; // Local increment
    ...
    try {
        ...
        console.log(`✅ Reacción ${type} guardada en Firestore`);
    } catch (error) {
    ...
    if (window.NotificationService && post.author?.id && post.author.id !== AppState.currentUser.uid) {
        window.NotificationService.send('POST_LIKE', {
            ...
            message: `${AppState.currentUser.name} reaccionó a tu publicación.`,
...
async function addComment(postId, event) {
    if (event.key === 'Enter') {
        ...
        if (post) {
            ...
            // NOTIFICATION TRIGGER DEBUG
            console.log('🔍 Checking Notification Trigger:', {
                hasService: !!window.NotificationService,
                authorId: post.author?.id,
                currentId: AppState.currentUser.uid,
                isNotSelf: post.author?.id !== AppState.currentUser.uid
            });

            if (window.NotificationService && post.author?.id && post.author.id !== AppState.currentUser.uid) {
                console.log('🚀 Triggering POST_COMMENT notification...');
                window.NotificationService.send('POST_COMMENT', {
                    targetUserId: post.author.id,
                    actor: {
                        id: AppState.currentUser.uid,
                        name: AppState.currentUser.name,
                    ...
                    message: `${AppState.currentUser.name} comentó tu publicación.`,
                    data: { postId: post.id, commentPreview: text.substring(0, 50) }
                });
            }
        }
    }
...
async function reactToComment(postId, commentId, type) {
    ...
    if (comment) {
        ...
        try {
            ...
            console.log('✅ Reacción a comentario guardada en Firestore');
        } catch (error) {
            console.error('Error guardando reacción a comentario:', error);
        }
...
async function submitReply(postId, commentId, event) {
    if (event.key === 'Enter') {
        const text = event.target.value.trim();
        if (!text) return;
