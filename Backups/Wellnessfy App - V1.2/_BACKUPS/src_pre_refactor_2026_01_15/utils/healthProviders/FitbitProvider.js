...
export class FitbitProvider extends HealthProvider {
    ...
    async getSleep(date) {
        const dateStr = this.formatDate(date);
        const fitbitUrl = `${this.baseUrl}/user/-/sleep/date/${dateStr}.json`;
        const proxyUrl = `https://corsproxy.io/?${encodeURIComponent(fitbitUrl)}`;
        ...
        return {
            duration: sleep.duration / 60000, // ms a minutos
    ...
    normalizeActivity(fitbitActivity) {
        return {
            ...
            name: fitbitActivity.activityName,
            type: this.mapActivityType(fitbitActivity.activityTypeId),
            startTime: new Date(fitbitActivity.startTime),
            endTime: new Date(new Date(fitbitActivity.startTime).getTime() + fitbitActivity.duration),
